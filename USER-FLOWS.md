...
| UF-11.2 | Mask caching (versioned smart load) | `loadSessionMasksSmart` in `api.ts` fetches per-frame version numbers, compares them against IDB-cached versions via `findStaleFrames`, and fetches only stale/missing frames (full fetch if >30% stale or <20 frames); results are stored in IDB via `putMasks`. Partial results (click/box responses, propagation SSE events) carry only the objects involved and are written through via `mergeMasks` in `maskCache.ts`, which merges into the existing cached frame record (memory first, IDB fallback) — never replacing it. | Cached masks must never serve a version older than the server's current version; the version key in IDB matches the server's `versions` response. A cache write for a subset of objects must never erase other objects' masks from the cached frame record (regression 2026-06-11: propagating one object replaced each touched frame's cache record with only that object, then the post-propagation version stamp marked the records fresh — on reopen, all other objects' masks appeared deleted despite being intact on the server). |
