...
**Must NOT:**
- Lose the detected objects when `handleDetect` auto-creates a class (no class pre-selected, no name match — the common first-time path). `createClass` writes `state.json` server-side and bumps its `version`; `handleDetect` must adopt that bump (`stateVersionRef.current += 1`) and pass the new class in the `persistState` payload. Otherwise the follow-up `persistState` PUT carries the stale version, 409s, and `reconcileFromServer` overwrites in-memory state with the server snapshot — which has the new class but not the just-detected objects — wiping them (regression 2026-06-14: "first text detect on an empty session → objects found but unassigned"). Same desync applies to `handleOrphanCreateClass`. Verify: on a fresh session, Detect with no class selected → after detection `state.json` contains both the new class and the detected objects, and no `state_version_conflict` 409 was returned.
...
**Contract:**
- **(a) Backend restart:** The frontend polls `GET /api/status` (via `useServiceStatus`) every few seconds. Each response includes a `boot_id` (a UUID generated at Flask startup in `app/config.py`). When the phase drops from `"ready"` to `"idle"` (detected by `prevPhaseRef` in `App.tsx`), the frontend resets `sessionId` to `null` **and clears BOTH load guards (`sessionLoadedRef` and `hasLoadedSessionRef`)**, then sets the status bar to `"Session lost — server restarted. Please select a session."` — the session list re-appears. Persisted session files (`masks.json`, `state.json`, `prompts.json`) are untouched by the restart; resuming the session via the list restores all prior annotations. **Must NOT** leave `hasLoadedSessionRef` pointing at the lost session — the auto-load effect is gated on `hasLoadedSessionRef.current !== session_id`, so a stale guard makes resuming the *same* session (after recycle/scale-to-zero/deploy) short-circuit `loadSession`, stranding the UI on the session list until a manual page refresh (regression 2026-06-14).
...
| UF-1.4 | Delete session | Clicking the trash icon on a session card calls `DELETE /api/video/sessions/<session_id>`; the backend removes the session directory from disk (and the GCS prefix in cloud mode); the frontend evicts any cached frames from IndexedDB via `evictSession` in `maskCache.ts` and removes the card from the list. Deletion is idempotent: in cloud mode `delete_session_gcs` tolerates a blob vanishing between its list snapshot and the per-blob delete (concurrent sync flush or repeat delete) — it catches `NotFound` rather than 500ing (regression 2026-06-14: double-DELETE of a just-closed session 500'd). Note the sessions list has a 30 s TTL cache (`gcs_storage._SESSIONS_CACHE_TTL`), so a just-deleted session can linger as a phantom row up to 30 s even after `invalidate_sessions_cache`. | Deleting session A must not touch the session directories or IndexedDB entries of any other session. A repeated or concurrent delete of the same session must not 500. |
...
| UF-11.2 | Mask caching (within-session accelerator) | Session open (`loadSession` in `App.tsx`) always re-syncs from the server: it evicts the session's IDB mask records (`evictSession`) and full-fetches `GET /api/session/masks/<sid>` — the cache is never trusted across opens. Within a session, partial results (click/box responses, propagation SSE events) carry only the objects involved and are written through via `mergeMasks` in `maskCache.ts`, which merges into the existing cached frame record (memory first, IDB fallback) — never replacing it. The versioned fast path (`loadSessionMasksSmart` in `api.ts`: per-frame versions compared via `findStaleFrames`, stale/missing frames fetched individually) backs the Refresh button (UF-11.3). | Opening a session must render exactly the server's masks regardless of prior cache contents (regression 2026-06-11: poisoned records version-stamped fresh were served on reopen, showing masks as deleted despite being intact on the server). A cache write for a subset of objects must never erase other objects' masks from the cached frame record (regression 2026-06-11: propagating one object replaced each touched frame's cache record with only that object). |
