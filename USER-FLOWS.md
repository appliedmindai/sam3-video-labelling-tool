...
6. `[BROWSER]` Press Ctrl+Z (or Cmd+Z) once → Expect: the click dot disappears; if other click points remain for this object on this frame, the mask updates to reflect the remaining points; if no points remain, the point dot and mask overlay both disappear from the canvas, the "Missing Mask" panel does NOT appear (both prompt and mask were cleared from React state), and `masks.json` on disk still contains the undone mask.
...
5. `[BROWSER]` Click the toolbar's Cancel button during a detect request → Expect: status bar shows "Detection cancelled" within 10 s of cancellation; no new objects appear in the object list in the frontend (the frontend discards the result); note that backend may have completed and written to disk — verify by checking `state.json` for any new entries after cancel (acceptable behavior, documented limitation above). Note: Escape is wired only to multi-select clearing, not to detection cancellation.
